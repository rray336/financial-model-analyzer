...
class FormulaAnalyzer:
    ...
    def build_dependency_tree(self, workbook_path: Path, sheet_name: str, 
                            cell_address: str, max_depth: int = 3) -> Optional[FormulaComponent]:
    ...
    def calculate_variance_attribution(self, old_component: FormulaComponent,
                                     new_component: FormulaComponent) -> DrillDownResult:
        """Calculate how each dependency contributes to the overall variance"""
        
        total_variance = new_component.value - old_component.value
        drill_down_path = [old_component.name]
        
        # Match dependencies between old and new
        matched_components = []
        explained_variance = 0.0
        
        # Simple matching by position/name
        old_deps = {dep.cell_reference: dep for dep in old_component.dependencies}
        new_deps = {dep.cell_reference: dep for dep in new_component.dependencies}
        
        for ref in old_deps:
            if ref in new_deps:
                old_dep = old_deps[ref]
                new_dep = new_deps[ref]
                dep_variance = new_dep.value - old_dep.value
                
                # Create matched component with variance
                matched_comp = FormulaComponent(
                    name=old_dep.name,
                    cell_reference=old_dep.cell_reference,
                    value=new_dep.value,
                    formula=old_dep.formula,
                    is_leaf_node=old_dep.is_leaf_node,
                    variance_contribution=dep_variance
                )
                
                matched_components.append(matched_comp)
                explained_variance += dep_variance
        
        unexplained_variance = total_variance - explained_variance
        
        return DrillDownResult(
            source_item=old_component.name,
            source_value=old_component.value,
            components=matched_components,
            total_explained=explained_variance,
    ...
    def analyze_formula_complexity(self, formula: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "can_drill_down": ref_count > 0,
            "reference_count": ref_count,
            "main_function": main_function,
            "has_cross_sheet_refs": has_cross_sheet,
            "has_external_refs": has_external_refs,
            "estimated_depth": min(3, ref_count // 2) if ref_count > 0 else 0
        }
